...
const SlideContent = ({ movie }: { movie: FeaturedMovie }) => {
  return (
    <div className="flex w-full flex-col justify-center rounded-sm border border-white/5 bg-zinc-900/30 p-8 md:p-12 lg:w-1/2">
      ...
      <h3 className="mb-8 text-4xl font-bold leading-none text-white md:text-6xl">
        {movie.titleMain} <span className="text-yellow-500">{movie.titleAccent}</span>
      </h3>

      <p className="mb-10 text-lg font-light leading-relaxed text-gray-400">{movie.description}</p>
      ...
      <div className="mb-12 grid grid-cols-2 gap-x-4 gap-y-8 border-t border-white/10 pt-8">
        ...
        <div>
          ...
          <p className="font-medium text-white">{movie.runtime}</p>
        </div>
        <div>
...
const SlidePoster = ({ movie }: { movie: FeaturedMovie }) => {
  return (
    <div className="group min-h-[400px] w-full overflow-hidden rounded-sm shadow-2xl md:min-h-[600px] lg:w-1/2">
      <div className="relative h-full min-h-[400px] w-full md:min-h-[600px]">
        <img
          src={movie.posterUrl}
          alt={`${movie.titleMain} ${movie.titleAccent}`}
        ...
        <div className="overflow-hidden">
          ...
          >
            {featuredMovies.map((movie, index) => (
              <div key={index} className="w-full shrink-0 min-h-[600px] md:min-h-[700px] lg:min-h-[650px]">
                <div className="flex h-full w-full flex-col items-center gap-12 lg:flex-row lg:items-stretch">
                  {index === 1 ? (
                    <>
                      <SlideContent movie={movie} />
                      <SlidePoster movie={movie} />
                    </>
                  ) : (
                    <>
                      <SlidePoster movie={movie} />
          ...
          {featuredMovies.map((_, index) => (
            <button
              ...
              className={`h-2.5 w-8 rounded-full transition-all duration-300 ${
                currentSlide === index ? "bg-yellow-500" : "bg-white/20 hover:bg-white/40"
              }`}
            />
          ))}
        </div>
